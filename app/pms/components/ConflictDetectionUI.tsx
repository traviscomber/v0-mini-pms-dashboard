...
export default function ConflictDetectionUI({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-lg p-6">
        ...
        {viewMode === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className={`p-4 rounded-lg ${totalConflicts > 0 ? 'bg-destructive/10' : 'bg-chart-2/10'}`}>
              <p className={`text-xs ${totalConflicts > 0 ? 'text-red-700' : 'text-green-700'}`}>
                Detected Conflicts
              </p>
              <p className={`text-2xl font-bold mt-1 ${totalConflicts > 0 ? 'text-destructive' : 'text-green-600'}`}>
                {Math.round(totalConflicts)}
              </p>
            </div>
            <div className={`p-4 rounded-lg ${hasProposedConflict ? 'bg-destructive/10' : 'bg-chart-2/10'}`}>
              ...
              <div className="flex items-center gap-2 mt-1">
                {hasProposedConflict ? (
                  <>
                    ...
                    <span className="text-sm font-semibold text-destructive">Conflict</span>
                  </>
                ) : proposedCheckIn ? (
                  <>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {proposedConflicts.map((conflict) => (
                <div key={conflict.id} className="p-3 bg-background rounded-lg border border-destructive/30">
      ...
      {/* Prevention Status */}
      <div className="bg-chart-2/10 border border-chart-2/50 rounded-lg p-4">
