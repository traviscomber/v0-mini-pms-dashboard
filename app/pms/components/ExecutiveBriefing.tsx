...
export default function ExecutiveBriefing({ isLoading = false, reservations, rooms, tasks }: ExecutiveBriefingProps) {
  ...
  const briefing = useMemo(() => {
    ...
    const occupancyRisk = occupancyRate < occupancyTarget ? (occupancyTarget - occupancyRate) * 1.2 : 0;
    const revenueRisk = overdueBalances.length * 16 + Math.max(0, activeReservations.length - checkInsToday.length) * 2;
    const turnoverRisk = (checkInsToday.length + checkOutsToday.length) * 3 + openCleaningTasks.length * 8;
    const taskRisk = urgentTasks.length * 10;
    const healthScore = Math.max(0, Math.min(100, Math.round(100 - (occupancyRisk + revenueRisk + turnoverRisk + taskRisk))));
      ...
      overdueBalances.length > 0
        ...
        : openCleaningTasks.length > 0
          ...
          : occupancyRate < occupancyTarget
            ? language === 'es'
              ? `La ocupación está en ${occupancyRate}% y puede absorber más demanda.`
              : `Occupancy is ${occupancyRate}% and could absorb more demand.`
            : language === 'es'
              ? 'La operación está equilibrada. Mantén la disciplina y protege el día.'
    ...
    <section className="rounded-3xl border border-primary/15 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 text-slate-100 shadow-2xl shadow-slate-950/30">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-slate-400">{t('executive.focusLabel')}</p>
              <h3 className="mt-1 text-lg font-semibold text-white">{t('executive.nextBestActions')}</h3>
            </div>
          ...
          <div className="mt-3 space-y-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">
                {language === 'es' ? 'Objetivo' : 'Target'} {briefing.occupancyTarget}% | {briefing.urgentTasks.length} {language === 'es' ? `tarea urgente${briefing.urgentTasks.length === 1 ? '' : 's'}` : `urgent task${briefing.urgentTasks.length === 1 ? '' : 's'}`}
              </p>
            </div>
            ...
            <p className="text-sm leading-6 text-slate-300">
              {briefing.overdueBalances.length > 0
                ...
                : briefing.openCleaningTasks.length > 0
                  ? language === 'es'
                    ? `${briefing.openCleaningTasks.length} habitaciones todavía necesitan cierre de turno.`
                    : `${briefing.openCleaningTasks.length} rooms still need turnover work.`
                  : language === 'es'
                    ? 'No hay bloqueos inmediatos. Usa la ventana para proteger la ocupación de mañana.'
