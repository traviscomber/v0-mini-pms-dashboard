...
export function LoginShell({
  next, message, supabaseReady, signInAction, signUpAction,
}: LoginShellProps) {
  const [tab, setTab] = useState<"signin" | "signup">("signin");
  const canvasRef = useRef<HTMLCanvasElement>(null);

  // Kick off canvas animation after mount — no state that affects rendered HTML
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Canvas — always in the tree, imperative draw after mount */}
      <canvas
        ref={canvasRef}
        aria-hidden="true"
        style={{
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: "0.875rem", flexWrap: "wrap" }}>
            {STATS.map((s, i) => (
              <StatCard key={s.label} stat={s} delay={0.15 + i * 0.1} />
            ))}
          </div>
            ...
            {FEATURES.map((f, i) => (
              <li
                ...
                style={{
                  ...
                  animation: `lp-slide-right 0.45s ease ${0.35 + i * 0.08}s both`,
                }}
              >
                <span
                  aria-hidden="true"
...
interface AuthInputProps {
  ...
  autoComplete?: string; hint?: string; minLength?: number;
}
